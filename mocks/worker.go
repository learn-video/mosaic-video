...
//
//      mockgen -source worker/worker.go -package mocks
//
...
import (
        ...
        worker "github.com/mauricioabreu/mosaic-video/worker"
...
func (m *MockLocker) Obtain(ctx context.Context, key string, ttl time.Duration) (worker.Lock, error) {
        m.ctrl.T.Helper()
        ret := m.ctrl.Call(m, "Obtain", ctx, key, ttl)
        ret0, _ := ret[0].(worker.Lock)
        ret1, _ := ret[1].(error)
        return ret0, ret1
}
